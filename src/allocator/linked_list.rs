use core::{mem::{align_of, size_of}, alloc::{GlobalAlloc, Layout}, ptr::null_mut};
...
impl LinkedListAllocator {
    pub const fn new() -> Self {
        LinkedListAllocator { head: Node::new(0) }
    }
    ...
    fn find_region(&mut self, size: usize, align: usize)
        ...
        while let Some(ref mut region) = now.next {
            if let Ok(alloc_start) = Self::alloc_from_region(&region, size, align) {
                let next = region.next.take();
                let ret = Some((now.next.take().unwrap(), alloc_start));
                now.next = next;
                return ret;
            } else {
    ...
    fn alloc_from_region(region: &Node, size: usize, align: usize)
        ...
        if 0 < excess_size && excess_size < size_of::<Node>() {
            return Err(());
        }
        Ok(alloc_start)
    }
    ...
    fn size_align(layout: Layout) -> (usize, usize) {
        let layout = layout.align_to(align_of::<Node>())
